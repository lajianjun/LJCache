...
/**
 LJKVStorageItem is used by `LJKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface LJKVStorageItem :NSObject
...
/**
 Storage type, indicated where the `LJKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache),
 use LJKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
 use LJKVStorageTypeFile to get better performance.
 * You can use LJKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, LJKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    LJKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    LJKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    LJKVStorageTypeMixed = 2,
};

/**
 LJKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for LJKVStorage is `initWithPath:type:`.
 ...
 */
@interface LJKVStorage : NSObject
...
@property (nonatomic, readonly) LJKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(LJKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is LJKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is LJKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is LJKVStorageTypeMixed, then the item.value will be saved to file
...
/**
 ...
 LJKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is LJKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is LJKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is LJKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable LJKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable LJKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `LJKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<LJKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `LJKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<LJKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
