...
@implementation LJKVStorageItem
@end

@implementation LJKVStorage {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"LJKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];//自定义错误
    return [self initWithPath:@"" type:LJKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(LJKVStorageType)type{
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"LJKVStorage init error: invalid path: [%@].",path);
        return nil;
    }
    if (type > LJKVStorageTypeMixed) {
        NSLog(@"LJKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _trashQueue = dispatch_queue_create("com.blue.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"LJKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (NSString *)_dbJoinedKeys:(NSArray *)keys {// todo
...
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value fileName:(NSString *)fileName extendedData:(NSData *)extendedData {
    ...
    int timestamp = (int)time(NULL);// get system time
...
- (LJKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    LJKVStorageItem *item = [LJKVStorageItem new];
...
- (LJKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    LJKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            LJKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
